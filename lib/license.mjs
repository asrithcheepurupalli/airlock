...
// Derive the public key (jwk x) from the private key, so the server can verify
// tokens it minted without a separately-configured public key.
export function publicKeyX(privKeyB64) {
  const priv = crypto.createPrivateKey({ key: Buffer.from(privKeyB64, 'base64'), format: 'der', type: 'pkcs8' })
